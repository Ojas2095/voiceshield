"""
VoiceShield - Layer 1 Voice Diversity Sweep (Phase A.4)
======================================================
...
Measures and logs:
- hf_ratio (telephone band 2800-3400Hz vs 250-2200Hz carrier ratio)
- jitter (biological vocal fold micro-tremor: 0.8% - 3.8%)
- raw_cnn (mel-spectrogram artifact branch)
- ai_score (calibrated P(synthetic) in [0, 1])
...
def extract_acoustic_features(chunk_samples: np.ndarray, sr: int = 16000) -> tuple[float, float]:
    """Extract hf_ratio and pitch jitter exactly as done in inference."""
...
def evaluate_file(file_path: Path, clf: VoiceShieldClassifier) -> dict:
    ...
    for i in range(0, len(data), chunk_len):
        ...
        if len(chunk) < 16000:  # ignore short trailing slivers
            continue
...
def run_sweep():
    print("=" * 115)
    print("VOICESHIELD LAYER 1 ACOUSTIC AUTHENTICITY DIVERSITY SWEEP")
    print("=" * 115)
    ...
    test_suite = [
        ...
        # Human Scammer Recordings (Crucial: Layer 1 must classify as HUMAN, not AI!)
    ...
    # Add 5 held-out real voice samples from data/real/ if available
    held_out_samples = sorted(glob.glob(str(REPO_ROOT / "data/real/*.wav")))[:5]
    for p in held_out_samples:
        test_suite.append((Path(p), "HUMAN"))

    print(f"{'AUDIO FILE':35} | {'EXPECTED':12} | {'CHKS':4} | {'AVG-SCR':7} | {'PEAK-SCR':8} | {'AVG-HF':6} | {'JITTER':6} | {'VERDICT':12} | {'STATUS'}")
    print("-" * 115)
    ...
    for file_path, expected in test_suite:
        ...
        print(
            ...
            f"{res['avg_hf']:6.3f} | {res['avg_jitter']:6.3f} | {res['verdict']:12} | {status_str}"
        )

    print("=" * 115)
    if all_passed:
        print(f"🎉 VOICE DIVERSITY SWEEP PASSED: ALL {len(test_suite)} RECORDINGS CORRECTLY SEPARATED (0 FP, 0 FN)!")
        sys.exit(0)
    else:
        print(f"❌ VOICE DIVERSITY SWEEP FAILED with {len(failures)} errors:")
        for f in failures:
            print(f"  - {f}")
        sys.exit(1)
